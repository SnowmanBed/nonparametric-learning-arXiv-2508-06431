    ...
    for idx_g, g in enumerate(tqdm(g_list)):
        ...
        for idx_mu, mu in enumerate(mu_list):
            ...
            try:
                ...
                if correction == True:
                    phi_t1_vals_gaussian, optimal_bd = compute_gaussian_phi_at_t_optimal(mu / radius, g / radius, X_precision, X_samples, W_Gaussian, t = radius / kappa)
                    phi_t1_vals_gaussian *= 1 / np.exp( - ((1 - kappa) / kappa * radius)**2 / 2   )
                else:
                    phi_t1_vals_gaussian, optimal_bd = compute_gaussian_phi_at_t_optimal(mu / radius, g / radius, X_precision, X_samples, W_Gaussian, t = radius)
