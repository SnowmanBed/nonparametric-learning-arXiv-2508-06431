...
def compute_gaussian_phi_at_t_optimal(mu, nu, m, X_precision, X_samples, w_gaussian_values, t):
    """
    Computes the Gaussian kernel estimated characteristic function φ(t) at t = 1 (by default) with optimized bandwidth.
    """
    alpha = np.sqrt(mu**2 + nu**2)
    # Compute φ(t)
    ...
    optimal_bd = 2 / alpha**2 / t * np.sqrt( (1 - np.abs(phi_t1) **2) / ( 2 * len(X_samples) * np.abs(phi_t1) **2) )
